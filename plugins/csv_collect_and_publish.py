...
class MainThread(threading.Thread):
    def __init__(self, plugin_config, max_iteration=None):
        ...
        self.noisereducer_thread = NoiseReducer("main thread", self.plugin_config)
    ...
    def run(self):
        while self.is_run:
            if not self.noisereducer_thread.is_processing and \
                ...
                self.noisereducer_thread = NoiseReducer(str(self.counter), self.plugin_config)
...
class CollectAndPublish():
    def __init__(self, plugin_config):
        ...
        with open(config_file) as config:
            self.project_file_path = plugin_config
            self.plugin_config = json.load(config)
            self.now = datetime.datetime.now()
            self.time_stamp = '%d-%d-%d_%d-%d-%d' % (self.now.year, self.now.month, self.now.day,
    ...
    def getting_next_point(self, sample):
